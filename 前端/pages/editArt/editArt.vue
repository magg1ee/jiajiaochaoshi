<template>
    <view class="wrap">
        <view class="write_title">
            <input type="text" v-model="title" placeholder="请输入招聘要求或求职要求" />
        </view>
        <!-- 内容展示区 -->
        <view class="artList">
        ...
        <form @submit="submit">
            <view class="inputArea">
                <view class="addImg" @tap="addImg">+信用图</view>
                <view class="addText">
                    <textarea name="artText" maxlength="-1" v-model="inputContent" placeholder="请输入详细要求" />
                    <button type="primary" form-type="submit">插入</button>
                </view>
            </view>
        </form>
        <!-- 选择分类 -->
        ...
        <!-- 提交按钮 -->
        <view class="submitNow" v-if="artList.length > 0" @tap="submitNow">编辑要求</view>
    </view>
</template>
<script>
